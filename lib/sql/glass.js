		...
		(function() {
			this.queryOnePromise = function(wheres) {
				...
				return function(connect) {
					return utils.FunctionToPromise(self.queryOne(wheres), connect)
				}
			}
			...
			this.queryPromise = function(wheres, options) {
				var self = this;
				return function(connect) {
					return utils.FunctionToPromise(self.query(wheres, options), connect);
				}
			}
			...
			this.updatePromise = function(wheres, update) {
				var self = this;
				return function(connect) {
					return utils.FunctionToPromise(self.update(wheres, update), connect);
				}
			}
			...
			this.delPromise = function(wheres) {
				var self = this;
				return function(connect) {
					return utils.FunctionToPromise(self.del(wheres), connect);
				}
			}
			...
			this.addPromise = function(g) {
				...
				var self = this;
				return function(connect) {
					console.log(utils.FunctionToPromise(self.add(g),connect));
					return utils.FunctionToPromise(self.add(g),connect);
				}
				*/
				var self = this;
				return function(connect) {
